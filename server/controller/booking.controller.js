...
export const singleUserBookingDetails = async (req, res) => {
	try {
		...
		const userBookings = await Booking.find({ user: userId }).populate('user', 'fullname, email').populate('car');
...
export const allBookingDetails = async (req, res) => {
	try {
		const bookings = await Booking.find().populate('user', 'fullname, email').populate('car');
...
export const getAllCancelledBookings = async (req, res) => {
	try {
		const cancelledBooking = await Booking.find({ status: 'cancelled', cancelledBy: 'user' }).populate('user', 'email').populate('car', 'model');
...
export const checkAvailability = async (req, res) => {
	try {
		const { tripStartDate, tripEndDate, userId } = req.body;
...
export const totalRevenue = async (req, res) => {
	try {
		...
		const { totalRevenue, totalRefunded } = bookings.reduce(
			(acc, booking) => {
